// main.ts - Deno Deploy Entry Point (Full Secure Version - Fixed Expiry System)
...
function getConfig() {
  ...
  const validFrom = Deno.env.get("VALID_FROM") || "2026-03-05"; // YYYY-MM-DD format
  const validUntil = Deno.env.get("VALID_UNTIL") || "2026-03-12"; // YYYY-MM-DD format
  ...
  // Key version - change this when you update keys to reset rate limits
  const keyVersion = Deno.env.get("KEY_VERSION") || "v1";

  return { keys, validFrom, validUntil, validityText, maxPerPeriod, keyVersion };
}
...
function isWithinValidPeriod(config: ReturnType<typeof getConfig>): boolean {
  const now = new Date();
  const from = new Date(config.validFrom + "T00:00:00Z");
  const until = new Date(config.validUntil + "T23:59:59Z");
...
async function incrementRateLimitAtomic(
  ...
  // Calculate expiry: time until validUntil + 1 day buffer
  const until = new Date(config.validUntil + "T23:59:59Z");
  const expireIn = Math.max(until.getTime() - Date.now() + 86400000, 86400000); // At least 1 day
...
async function getRandomKey(config: ReturnType<typeof getConfig>): Promise<{ key: string } | null> {
...
<script>
  ...
  async function checkRemaining() {
    try {
      ...
      document.getElementById('statStatus').style.color = data.withinPeriod ? 'var(--success-light)' : '#fca5a5';
      ...
      if (!data.allowed) {
        ...
        btn.disabled = true;
        if (!data.withinPeriod) {
          document.getElementById('btnText').textContent = 'Key သက်တမ်း ကုန်ဆုံးနေပါသည်';
        } else {
          document.getElementById('btnText').textContent = 'Generate ခွင့် ကုန်သွားပါပြီ';
        }
      }
    } catch (e) {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (!data.success) {
        ...
        if (data.error === 'limit_reached' || data.error === 'expired') {
          btn.disabled = true;
          btnText.textContent = data.error === 'expired' ? 'Key သက်တမ်း ကုန်ဆုံးနေပါသည်' : 'Generate ခွင့် ကုန်သွားပါပြီ';
        ...
        if (remaining <= 0) {
          btn.disabled = true;
          btnText.textContent = 'Generate ခွင့် ကုန်သွားပါပြီ';
          spinner.style.display = 'none';
          showSuccess();
          await checkRemaining();
          isGenerating = false;
    ...
    const currentBtnText = document.getElementById('btnText').textContent;
    if (!currentBtnText.includes('ကုန်သွားပါပြီ') && !currentBtnText.includes('ကုန်ဆုံး')) {
      btn.disabled = false;
    }
    lucide.createIcons();
    isGenerating = false;
