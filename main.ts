...
async function handleGenerate(req: Request): Promise<Response> {
  ...
  try {
    ...
    if ((body.website as string)?.length > 0 || (body.email as string)?.length > 0) {
      return jsonResponse({ success: true, payload: btoa("fake-payload-" + Math.random()), remaining: 0 }); 
    }
...
<script>
      ...
      function tryNonces(start, count, chal, pfx, cb) {
        ...
        for (var i=start; i<start+count; i++) {
          promises.push(crypto.subtle.digest('SHA-256', new TextEncoder().encode(chal+'||'+i)).then(function(hash) {
            ...
            return hex.startsWith(pfx) ? String(start + promises.indexOf(arguments[0]) ) : false; 
          }));
        }
        var realPromises =[];
  ...
  function checkRemaining() {
    fetch('/api/check', { method: 'POST', body: JSON.stringify({}) }).then(res => res.json()).then(data => {
      ...
      document.getElementById('btnText').textContent = !data.allowed ? (data.withinPeriod ? 'Keyယူခွင့် ကုန်သွားပါပြီ' : 'သက်တမ်းကုန်နေသည်') : 'Generate Key';
    }).catch(e=>{});
  }
