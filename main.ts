// ============== Patgaduu VLESS Key Generator ==============
// Deno Deploy - Full Stack Application
...
function getCSRFSecret(): string {
  ...
  if (!secret || secret.length < 32) {
    console.error("WARNING: CSRF_SECRET env variable is missing or too short. Set a strong random string (>=32 chars).");
  }
  return secret || "patgaduu-csrf-CHANGE-THIS-IN-PRODUCTION-" + "0".repeat(40);
}
...
async function generateServerFingerprint(ip: string, userAgent: string): Promise<string> {
  const salt = Deno.env.get("FINGERPRINT_SALT") || "patgaduu-fp-salt-2024";
...
function parseUTCFromLocal(dateStr: string, time: string, tzOffset: number): number {
  // Parse as explicit UTC offset to avoid server timezone inconsistency
...
// Atomic increment with built-in limit check (eliminates TOCTOU race condition)
async function incrementAllAtomic(
  ...
  for (let i = 0; i < maxRetries; i++) {
    ...
    // Check limits inside atomic loop (prevents TOCTOU)
  ...
  // Increment global total using KV U64 Mutate Sum (zero contention / thread safe)
  await kv.atomic()
    .mutate({
      type: "sum",
...
// Encrypt payload using a shared secret derived from env variable
// The key is NOT sent to the client — client must use the shared secret to decrypt
...
async function encryptPayload(plaintext: string): Promise<string> {
  ...
  // Only send IV + ciphertext (key is derived from shared secret, NOT sent)
...
function getClientIP(req: Request): string {
  ...
  // these headers are set by the proxy and are trustworthy.
...
async function handleGenerate(req: Request): Promise<Response> {
  ...
  // Atomic increment includes limit check (no TOCTOU race condition)
...
async function handleDebug(req: Request): Promise<Response> {
  ...
  // Use Authorization header instead of query param to avoid URL logging leaks
...
function getHTML(): string {
  ...
  // Escape all user-configurable values before embedding in HTML
  ...
  // Build the CSP nonce for inline script
...
<head>
  ...
  <style nonce="${cspNonce}">
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .notice-icon { color: #b45309; flex-shrink: 0; }
    ...
    .notice-close { background: none; border: none; font-size: 20px; color: #b45309; cursor: pointer; opacity: 0.7; flex-shrink: 0; }
    ...
    .logo-icon { width: 38px; height: 38px; background: linear-gradient(135deg, var(--primary), var(--purple)); border-radius: var(--radius-sm); display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 15px rgba(99,102,241,0.3); color: white; flex-shrink: 0; }
    ...
    .vn-icon { width: 40px; height: 40px; background: rgba(6,182,212,0.1); border-radius: var(--radius-sm); display: flex; align-items: center; justify-content: center; color: var(--cyan); flex-shrink: 0; }
    ...
    .tg-icon { width: 34px; height: 34px; background: rgba(6, 182, 212, 0.08); border-radius: var(--radius-sm); display: flex; align-items: center; justify-content: center; color: var(--cyan); flex-shrink: 0; }
    ...
    .tg-link:hover { background: rgba(6, 182, 212, 0.12); }
    ...
    .qr-modal-content, .success-popup { background: var(--bg-card); border-radius: var(--radius-xl); padding: 28px; text-align: center; max-width: 300px; width: 90%; box-shadow: 0 25px 50px rgba(0,0,0,0.15); animation: popIn 0.4s cubic-bezier(0.68, -0.55, 0.265, 1.55); }
...
<body>
  ...
  <div class="container">
    ...
    <div class="main-card">
      ...
      <div class="info-bars">
        <div class="info-bar"><div class="label">မိမိGenerateယူခွင့် ကျန်ရှိအကြိမ်</div><div class="count" id="remainingCount" style="color:var(--accent-light);">-</div></div>
        <div class="info-bar"><div class="label">Userများ စုစုပေါင်း Generateထားသော အကြိမ်</div><div class="count" id="totalCount">-</div></div>
      </div>
...
(function() {
  ...
  var PAYLOAD_SECRET = ''; // Will be set from server config if needed
  ...
  // ---- Proof of Work Solver (optimized, non-blocking) ----
  ...
  function checkRemaining() {
    ...
    .then(function(data) {
      ...
      // Update TG info from server (escaped via textContent)
  ...
  function handleGenerate() {
    ...
    .then(function(data) {
      ...
      // Derive same key as server using PBKDF2 from shared secret
  ...
  function deriveDecryptionKey() {
    // Must match server's deriveEncryptionKey()
    // The PAYLOAD_SECRET is embedded server-side for the client
  ...
  function resetButton() {
    ...
    isGenerating = false;
    // Re-enable will happen via checkRemaining() callback
    // But set a fallback in case checkRemaining fails
    setTimeout(function() {
      if (!isGenerating) {
  ...
  // Expose closeNotice for the notice bar
  window.closeNotice = function() {
    var el = document.getElementById('adminNoticeSlider');
    if (el) el.style.display = 'none';
  };

  // Shared secret for payload decryption (embedded by server)
  var PATGADUU_SHARED_SECRET = '${escapeHTML(Deno.env.get("PAYLOAD_CLIENT_SECRET") || "patgaduu-client-shared-secret-change-me")}';
...
Deno.serve(async (req) => {
  ...
  const securityHeaders: Record<string, string> = {
    "X-Frame-Options": "DENY",
    "X-Content-Type-Options": "nosniff",
    "X-XSS-Protection": "0", // Deprecated, CSP is used instead
    "Referrer-Policy": "strict-origin-when-cross-origin",
    "Permissions-Policy": "camera=(), microphone=(), geolocation=(), payment=()",
    "Strict-Transport-Security": "max-age=63072000; includeSubDomains; preload",
    "Content-Security-Policy": [
      "default-src 'none'",
      "script-src 'self' https://unpkg.com 'unsafe-inline'", // unsafe-inline needed for nonce — ideally use nonce CSP
  ...
  // Block common attack paths early
  ...
  // Only allow GET/HEAD for HTML page
