// main.ts - Deno Deploy Entry Point (Full Secure Version - Fixed V3)
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* AOS fallback - ensure elements are visible even if AOS fails to load */
    [data-aos] {
      opacity: 1 !important;
...
Deno.serve(async (req) => {
  ...
  return new Response(getHTML(), {
    headers: {
      ...
      "Content-Security-Policy": "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://unpkg.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://unpkg.com; font-src https://fonts.gstatic.com; img-src 'self' data: blob:; connect-src 'self';",
      ...securityHeaders,
    }
  });
});
